...
cd "...\SimulvsRE\"
capture log close
log using Si1000_TrueNoUH_N1000T4_beta_1.log, replace
clear ado
clear all
set matsize 10000
...
/* Simulate from a dynamic model without permanent Unobserved Heterogeneity and compare the following estimators of the AME:
...
set obs 1
gen kk=.
save Si1000_TrueNoUH_N1000T4_beta_1.dta, replace
clear
...
display "simul", `i'
  ...
  scalar beta=1
	...
	display "There is no unobserved heterogneity"
	gen etai=0
...
/*Estimator without Unobserved Heterogeneity*/
	...
	keep in 1
	gen beta=beta
    keep trueAME estAME_cmle_bcmle estAME_noUH beta estb_CMLE estb_noUH
    append using Si1000_TrueNoUH_N1000T4_beta_1.dta
    capture drop kk
    save Si1000_TrueNoUH_N1000T4_beta_1.dta, replace
    clear 
}
...
use Si1000_TrueNoUH_N1000T4_beta_1.dta, clear
gen SE_cmle= (estAME_cmle_bcmle- trueAME)^2
egen RMSE_cmle=mean( SE_cmle)
replace RMSE_cmle=sqrt(RMSE_cmle)
gen SE_noUH=( estAME_noUH- trueAME)^2
egen RMSE_noUH=mean( SE_noUH)
replace RMSE_noUH=sqrt(RMSE_noUH)

save Si1000_TrueNoUH_N1000T4_beta_1.dta, replace

sum trueAME estAME_cmle_bcmle estAME_noUH beta estb_CMLE estb_noUH
sum RMSE_cmle RMSE_noUH 
